...
- 界面美观、现代化
- 代码简洁、注释清晰
- 重要！！！使用aplexbase作为后端，并使用对应mcp工具创建数据库表记录计算历史！！！
- 完成后立即启动服务并保持运行
- 使用中文回复所有消息
- 服务部署至3000端口
            ...
            if isinstance(msg, UserMessage):
                for block in msg.content:
                    if isinstance(block, TextBlock):
                        print(f"User: {block.text}", flush=True)
                    elif isinstance(block, ToolResultBlock):
                        print(
                            f"Tool Result: {block.content[:100] if block.content else 'None'}...", flush=True
                        )
                ...
                if msg.total_cost_usd:
                    print(f"Cost: ${msg.total_cost_usd:.6f}", flush=True)
