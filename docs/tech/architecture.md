...
本文档描述了如何使用 E2B Template Python API 替代 Dockerfile，并在 E2B Sandbox 中运行 Claude Agent SDK 的完整技术架构，包括 AIPEXBASE 后端集成和 MCP 服务器配置。
...
graph TB
    ...
    subgraph "3. E2B Sandbox 实例"
        ...
        K --> MCP[MCP 工具<br/>execute_sql/list_tables]
        M --> N[生成文件<br/>index.html等]
    end

    subgraph "4. AIPEXBASE 后端"
        MCP --> AIPEX[AIPEXBASE MCP Server]
        AIPEX --> DB[(MySQL Database)]
        AIPEX --> API[RESTful API]
    end
    ...
    subgraph "6. 系统提示词"
        PROMPT[prompt.py<br/>系统提示词] -.嵌入.-> J
    end
...
**职责**: 通过 MCP 服务器为 Agent 提供数据库操作能力
...
3. **list_dynamic_api**: 列出自动生成的 API
   ```json
   {
...
**工作流程**:
1. Agent 分析任务,识别需要数据库操作
2. 调用 MCP 工具 `execute_sql` 创建表结构
3. AIPEXBASE 服务器接收请求并执行 SQL
4. 返回执行结果给 Agent
5. Agent 继续后续任务（如生成前端代码）

### 2.5 系统提示词层 (prompt.py)

**职责**: 为 Agent 提供 AIPEXBASE 应用开发规范和指导
...
# src/prompt.py
append_prompt = """
- 数据库设计规范（MySQL JSON Schema）
- DSL 类型系统（number/string/keyword/datetime等）
- aipexbase-js SDK 使用方法
- MCP 工具调用说明
"""
```
...
async def run_code_in_sandbox(code_file: str, env_vars: Optional[dict] = None) -> dict:
    ...
    async with SandboxManager(template_id, env_vars) as manager:
        # 上传代码文件
        await manager.sandbox.files.write(f"/home/user/{code_file}", code_content)

        # 执行代码
        result = await manager.sandbox.commands.run(f"python /home/user/{code_file}")
...
### 3.3 Agent-MCP-Database 交互流程

```mermaid
sequenceDiagram
    participant Agent as Claude Agent SDK
    participant MCP as MCP Tools
    participant AIPEX as AIPEXBASE Server
    participant DB as MySQL Database
    participant Frontend as 前端代码

    Note over Agent: 1. 分析任务需求
    Agent->>Agent: 设计数据库表结构
    
    Note over Agent,DB: 2. 创建数据库表
    Agent->>MCP: execute_sql("CREATE TABLE ...")
    MCP->>AIPEX: HTTP POST /mcp/sse
    AIPEX->>DB: 执行 SQL
    DB-->>AIPEX: 执行成功
    AIPEX-->>MCP: 返回结果
    MCP-->>Agent: 表已创建

    Note over Agent: 3. 查询可用 API
    Agent->>MCP: list_dynamic_api()
    MCP->>AIPEX: 获取 API 列表
    AIPEX-->>MCP: API 端点列表
    MCP-->>Agent: 返回 API 信息

    Note over Agent,Frontend: 4. 生成前端代码
    Agent->>Frontend: 生成 index.html
    Frontend->>Frontend: 使用 aipexbase-js SDK
    Frontend->>AIPEX: 前端调用 API
    AIPEX->>DB: 数据查询/更新
    DB-->>AIPEX: 返回数据
    AIPEX-->>Frontend: JSON 响应

    Note over Agent: 5. 启动 Web 服务
    Agent->>Agent: 执行 python -m http.server
```
...
flowchart TD
    ...
    I --> J[调用 MCP: execute_sql]
    J --> K[AIPEXBASE 创建表]
    K --> L[查询可用 API]
    
    L --> M[生成前端代码 index.html]
    M --> N[使用 aipexbase-js SDK]
    N --> O[启动 HTTP 服务器]
...
template = Template().set_envs({
    ...
    "MCP_URL": "http://server/mcp/sse?token=xxx"  # 不安全！
})
...
template = Template().set_envs({
    ...
    "ANTHROPIC_AUTH_TOKEN": os.getenv("ANTHROPIC_AUTH_TOKEN", "")  # 安全
})

# ⚠️ 临时方案：agent_runner.py 中的硬编码 MCP URL
# 需要改进为从环境变量读取
```

### 5.2 MCP 服务器安全

- ✅ 使用 Token 认证（SSE Transport）
- ✅ HTTPS 加密传输（生产环境）
- ✅ 项目级别权限隔离
- ⚠️ Token 不应硬编码在代码中（待改进）
